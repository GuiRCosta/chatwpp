...
describe("FastReplyController Integration", () => {
  ...
  describe("GET /fast-replies", () => {
    it("returns 200 with list of fast replies", async () => {
      const mockFastReplies = [
        { id: 1, key: "/hello", message: "Hello there!" },
        { id: 2, key: "/bye", message: "Goodbye!" }
      ]
      vi.mocked(FastReplyService.listFastReplies).mockResolvedValue(mockFastReplies as never)
      ...
      expect(response.body.data).toEqual(mockFastReplies)
    })

    it("returns 401 without auth token", async () => {
      const response = await request(app).get("/fast-replies")

      expect(response.status).toBe(401)
      expect(response.body.success).toBe(false)
      expect(FastReplyService.listFastReplies).not.toHaveBeenCalled()
    })
  })

  describe("POST /fast-replies", () => {
    it("returns 201 with created fast reply", async () => {
      const mockFastReply = { id: 3, key: "/thanks", message: "Thank you!" }
      vi.mocked(FastReplyService.createFastReply).mockResolvedValue(mockFastReply as never)

      const response = await request(app)
        .post("/fast-replies")
        .set("Authorization", `Bearer ${token}`)
        .send({ key: "/thanks", message: "Thank you!" })

      expect(response.status).toBe(201)
      expect(response.body.success).toBe(true)
      expect(response.body.data).toEqual(mockFastReply)
    })
    ...
    it("returns error when key is missing", async () => {
      ...
      expect(response.body.success).toBe(false)
      expect(FastReplyService.createFastReply).not.toHaveBeenCalled()
    })
  })

  describe("PUT /fast-replies/:id", () => {
    it("returns 200 with updated fast reply", async () => {
      const mockFastReply = { id: 1, key: "/hello", message: "Updated greeting!" }
      vi.mocked(FastReplyService.updateFastReply).mockResolvedValue(mockFastReply as never)

      const response = await request(app)
        .put("/fast-replies/1")
        .set("Authorization", `Bearer ${token}`)
        .send({ message: "Updated greeting!" })
  ...
  describe("DELETE /fast-replies/:id", () => {
    it("returns 200 on successful deletion", async () => {
      vi.mocked(FastReplyService.deleteFastReply).mockResolvedValue(undefined)

      const response = await request(app)
        .delete("/fast-replies/1")
        .set("Authorization", `Bearer ${token}`)

      expect(response.status).toBe(200)
      expect(response.body.success).toBe(true)
      expect(response.body.data.message).toBe("Fast reply deleted successfully")
    })
  })
})
