...
describe("CallLogController Integration", () => {
  ...
  describe("GET /calllogs", () => {
    it("returns 200 with list of call logs", async () => {
      const mockCallLogs = [
        { id: 1, contactId: 1, type: "incoming", duration: 120 },
        { id: 2, contactId: 2, type: "outgoing", duration: 60 }
      ]
      vi.mocked(CallLogService.listCallLogs).mockResolvedValue(mockCallLogs as never)
      ...
      expect(response.body.data).toEqual(mockCallLogs)
    })

    it("returns 401 without auth token", async () => {
      const response = await request(app).get("/calllogs")

      expect(response.status).toBe(401)
      expect(response.body.success).toBe(false)
      expect(CallLogService.listCallLogs).not.toHaveBeenCalled()
    })
  })

  describe("GET /calllogs/:id", () => {
    it("returns 200 with call log details", async () => {
      const mockCallLog = { id: 1, contactId: 1, type: "incoming", duration: 120 }
      vi.mocked(CallLogService.findCallLogById).mockResolvedValue(mockCallLog as never)
      ...
      expect(response.status).toBe(200)
      expect(response.body.success).toBe(true)
      expect(response.body.data).toEqual(mockCallLog)
      expect(CallLogService.findCallLogById).toHaveBeenCalledWith(1, 1)
    })

    it("returns 404 when call log is not found", async () => {
      vi.mocked(CallLogService.findCallLogById).mockRejectedValue(
        new AppError("CallLog not found", 404)
      )
  ...
  describe("POST /calllogs", () => {
    it("returns 201 with created call log", async () => {
      const mockCallLog = { id: 3, contactId: 1, type: "outgoing", duration: 30 }
      vi.mocked(CallLogService.createCallLog).mockResolvedValue(mockCallLog as never)

      const response = await request(app)
        .post("/calllogs")
        .set("Authorization", `Bearer ${token}`)
        .send({ contactId: 1, type: "outgoing", duration: 30 })
  ...
  describe("DELETE /calllogs/:id", () => {
    it("returns 200 on successful deletion", async () => {
      vi.mocked(CallLogService.deleteCallLog).mockResolvedValue(undefined)
      ...
      expect(response.status).toBe(200)
      expect(response.body.success).toBe(true)
      expect(response.body.data.message).toBe("CallLog deleted successfully")
      expect(CallLogService.deleteCallLog).toHaveBeenCalledWith(1, 1)
    })
  })
})
