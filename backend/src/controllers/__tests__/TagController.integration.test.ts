...
describe("TagController Integration", () => {
  ...
  describe("GET /tags", () => {
    it("returns 200 with list of tags", async () => {
      const mockTags = [
        { id: 1, name: "VIP", color: "#FFD700" },
        { id: 2, name: "New", color: "#00FF00" }
      ]
      vi.mocked(TagService.listTags).mockResolvedValue(mockTags as never)
  ...
  describe("POST /tags", () => {
    it("returns 201 with created tag", async () => {
      const mockTag = { id: 3, name: "Priority", color: "#FF0000" }
      vi.mocked(TagService.createTag).mockResolvedValue(mockTag as never)

      const response = await request(app)
        .post("/tags")
        .set("Authorization", `Bearer ${token}`)
        .send({ name: "Priority", color: "#FF0000" })
  ...
  describe("PUT /tags/:id", () => {
    it("returns 200 with updated tag", async () => {
      const mockTag = { id: 1, name: "Updated VIP", color: "#FFD700" }
      vi.mocked(TagService.updateTag).mockResolvedValue(mockTag as never)

      const response = await request(app)
        .put("/tags/1")
        .set("Authorization", `Bearer ${token}`)
        .send({ name: "Updated VIP" })
  ...
  describe("DELETE /tags/:id", () => {
    it("returns 200 on successful deletion", async () => {
      ...
      expect(response.status).toBe(200)
      expect(response.body.success).toBe(true)
      expect(response.body.data.message).toBe("Tag deleted successfully")
      expect(TagService.deleteTag).toHaveBeenCalledWith(1, 1)
    })
  })
})
